...
function IconButton({ label, onClick, danger, children }) {
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={`rounded p-1.5 hover:bg-stone-100 ${danger ? 'text-[#6e2a1e] hover:bg-[#6e2a1e]/10' : 'text-stone-500'}`}
    >
      {children}
    </button>
  )
...
function ManageRow({ item, onRename, onArchive, onUnarchive, onDelete, blockedMessage }) {
    ...
    return (
      <form onSubmit={submitRename} className="flex items-center gap-1.5 py-1">
        ...
        <input
          ...
          onBlur={submitRename}
          className="w-full rounded-md border border-stone-300 bg-white px-2 py-1 text-xs text-stone-800 focus:border-[#33493c] focus:outline-none"
        />
        <button type="submit" className="shrink-0 text-xs font-medium text-[#33493c] hover:underline">
          {t('settings.save')}
        </button>
      </form>
    )
  ...
  return (
    <div className="flex items-center justify-between gap-2 py-1">
      <span className={`truncate text-xs ${item.actief ? 'text-stone-700' : 'text-stone-400 line-through'}`} title={item.naam}>
        {item.naam}
        {!item.actief && <span className="ml-1.5 rounded bg-stone-100 px-1 py-0.5 text-[10px] font-medium normal-case text-stone-500 no-underline">{t('settings.archived')}</span>}
      </span>
      ...
      {blocked && <p className="w-full text-[11px] text-[#6e2a1e]">{blockedMessage}</p>}
    </div>
  )
}
  ...
  return (
    <div className="rounded-md border border-stone-200">
      ...
      >
        ...
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className={`shrink-0 text-stone-400 transition-transform ${open ? 'rotate-180' : ''}`}>
          <path d="m6 9 6 6 6-6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>
      {open && (
        <div className="border-t border-stone-100 px-3 py-2">
          {helper && <p className="mb-2 text-[11px] leading-relaxed text-stone-400">{helper}</p>}
          <div className="max-h-48 divide-y divide-stone-50 overflow-y-auto">
            {sorted.length === 0 && <p className="py-1.5 text-xs text-stone-400">{t('settings.empty')}</p>}
            {sorted.map((item) => (
              <ManageRow
                key={item.id}
                item={item}
          ...
          <form onSubmit={submitAdd} className="mt-2 flex items-center gap-1.5 border-t border-stone-100 pt-2">
            ...
            <input
              ...
              className="w-full rounded-md border border-stone-300 bg-white px-2 py-1.5 text-xs text-stone-800 placeholder:text-stone-400 focus:border-[#33493c] focus:outline-none"
            />
            <button type="submit" className="shrink-0 rounded-md bg-[#33493c] px-2.5 py-1.5 text-xs text-white hover:bg-[#263a2f]">
              {t('settings.add')}
            </button>
          </form>
        </div>
...
export default function SettingsPanel({ onClose, onExportPng }) {
  ...
  return (
    <div
      ...
      className="absolute right-0 top-full z-50 mt-2 w-96 rounded-xl border border-stone-200 bg-white shadow-xl"
    >
      <div className="flex items-center justify-between border-b border-stone-100 px-4 py-3">
        <h3 className="text-sm font-semibold text-stone-800">{t('settings.title')}</h3>
        <button type="button" onClick={onClose} aria-label={t('nav.close')} className="text-stone-400 hover:text-stone-600">
          ✕
        </button>
      </div>
      ...
      <div className="max-h-[70vh] space-y-4 overflow-y-auto px-4 py-3.5">
        ...
        <div className="rounded-md border border-stone-200 px-3 py-2.5">
          ...
          {!usingMockData && (
            <button type="button" onClick={loadMockData} className="mt-2 text-xs font-medium text-[#33493c] hover:underline">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          />
          {importError && (
            <p role="alert" className="rounded-md bg-[#6e2a1e]/5 px-2.5 py-2 text-xs text-[#6e2a1e]">
              {importError}
            </p>
          )}
        </div>
